...
function ForYouPosts() {
  ...
  const currentPosts = useQuery({
    ...
    queryFn: () => getForYouPosts({ page }),
  });

  useEffect(() => {
    if (currentPosts.status === 'success') {
      console.log('setting posts with new data');
      setPosts((posts) => [...posts, ...currentPosts.data.posts]);
      setLoadingPosts([]);
    }
  ...
  const allItems = [...posts, ...loadingPosts];
    ...
    <div ref={parentRef}>
      ...
      >
        {virtualizer.getVirtualItems().map((virtualRow) => {
          ...
          const isPost = virtualRow.index < posts.length;
